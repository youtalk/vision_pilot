...
"$PKG/CarlaUnreal.sh" "$MAP" -RenderOffScreen -nosound --ros2 --rmw=cyclonedds --ros-domain-id=1 \
  -ExecCmds="r.DefaultFeature.AutoExposure 0" > "$LOG" 2>&1 &
pid=$!
for _ in $(seq 1 120); do
  if (exec 3<>/dev/tcp/127.0.0.1/2000) 2>/dev/null; then
    log=$(cat "$LOG")
    grep -q "ROS2: enabled with middleware 'cyclonedds'" <<<"$log" || { kill "$pid"; echo "CARLA_SERVER_FAIL reason=ros2_not_enabled"; exit 1; }
    echo "CARLA_SERVER_UP pid=$pid"; exit 0
  fi
  kill -0 "$pid" 2>/dev/null || { echo "CARLA_SERVER_FAIL reason=server_exited log=$LOG"; exit 1; }
  sleep 2
done
kill "$pid"; echo "CARLA_SERVER_FAIL reason=port_timeout"; exit 1
